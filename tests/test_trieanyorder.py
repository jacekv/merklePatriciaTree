...
class TrieAnyOrder(TestCase):
    ...
    def test_singleItem(self):
        test = [[b"A", b"aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"]]
    ...
    def test_dogs(self):
        test = [[b"doe", b"reindeer"], [b"dog", b"puppy"], [b"dogglesworth", b"cat"]]
        h = self._feed_trie(test)
        self.assertTrue(
            h.hex()
    ...
    def test_foo(self):
        test = [[b"foo", b"bar"], [b"food", b"bass"]]
        h = self._feed_trie(test)
        self.assertTrue(
            h.hex()
    ...
    def test_smallValues(self):
        test = [[b"be", b"e"], [b"dog", b"puppy"], [b"bed", b"d"]]
        h = self._feed_trie(test)
        self.assertTrue(
            h.hex()
    ...
    def test_testy(self):
        test = [[b"test", b"test"], [b"te", b"testy"]]
        h = self._feed_trie(test)
        self.assertTrue(
            h.hex()
